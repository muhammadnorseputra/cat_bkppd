...
class Adm extends CI_Controller {
	...
	public function m_siswa() {
		$this->cek_aktif();
		cek_hakakses(array("admin","registrasi"), $this->session->userdata('admin_level'), $this->session->userdata('username'));
		...
		if ($uri3 == "det") {
	        ...
	        $q_datanya = $this->db->query("SELECT a.*,
											...
											(SELECT password FROM m_admin WHERE level = 'siswa' AND kon_id = a.id) AS pass
											FROM m_siswa a
	        ...
	        foreach ($q_datanya as $d) {
	        	$pass = (base64_decode($d['pass']) == 0) ? '<label class="label label-danger"> null </label>' : base64_decode($d['pass']); 
	            $data_ok = array();
	            $data_ok[] = $no++;
	            $data_ok[] =  $pass;
	            $data_ok[] = $d['nama'];
			...
			if (!empty($q_get_user)) {
				foreach ($q_get_user as $j) {
					if ($j['usernya'] == "N") {
						$this->db->query("INSERT INTO m_admin VALUES (null, '".$j['nim']."','". base64_encode($j['nim'])."', 'siswa', '".$j['id']."')");
						$jml_aktif++;
					}
				}
			}
		...
		if ($uri3 == "det") {
			...
			if (!empty($det_user)) {
				...
				if ($q_cek_username < 1) {
					...
					$this->db->query("INSERT INTO m_admin VALUES (null, '".$det_user->nip."', '".base64_encode($det_user->nip)."', 'guru', '".$det_user->id."')");
					$ret_arr['status'] 	= "ok";
					$ret_arr['caption']	= "tambah user sukses";
					j($ret_arr);
				} else {
			...
			$this->db->query("UPDATE m_admin SET password = '".base64_encode($det_user->nip)."' WHERE level = 'guru' AND kon_id = '".$det_user->id."'");
			...
			if (!empty($q_get_user)) {
				foreach ($q_get_user as $j) {
					if ($j['usernya'] == "N") {
						$this->db->query("INSERT INTO m_admin VALUES (null, '".$j['nip']."','".base64_encode($j['nip'])."', 'guru', '".$j['id']."')");
						$jml_aktif++;
					}
				}
			}
	...
	public function h_ujian() {
		...
		if ($uri3 == "det") {
	        ...
	        $q_datanya = $this->db->query("SELECT a.*, a.id as idtes, b.nama AS mapel, c.nama AS nama_guru FROM tr_guru_tes a
	        	INNER JOIN m_mapel b ON a.id_mapel = b.id 
	        	INNER JOIN m_guru c ON a.id_guru = c.id
	...
	public function ikut_ujian() {
		...
		if ($uri3 == "simpan_satu") {
			...
			$nilai_bobot = ($nilai / $total_bobot) * $nilai_bobot;
	...
	public function daftar() {
		// $provinsi = $this->curl->simple_get('http://dev.farizdotid.com/api/daerahindonesia/provinsi');
		$this->session->set_userdata(array('nik' => "", 'nama' => ""));
		// $data = [
		// 	'list_provinsi' => json_decode($provinsi)
		// ];
		$this->load->view('aaa_daftar'); 
	}
	
	public function list_provinsi() {
		$provs = $this->curl->simple_get('http://dev.farizdotid.com/api/daerahindonesia/provinsi');
		$json = json_decode($provs);
		$val = '<option value="">-- Pilih Provinsi --</option>';
		if(count($json) > 0) {
			foreach ($json->provinsi as $prov) {
				# code...
				$val .= "<option value='".$prov->id."'>".$prov->nama."</option>";
			}
		} else {
			$val = "<option value=''>Data Provinsi Tidak Ditemukan</option>";
		}
		echo $val;
		// var_dump(count($json->kabupatens));
	}

	public function list_kabupaten() {
		$id = $this->input->get('id');
		$kabs = $this->curl->simple_get('http://dev.farizdotid.com/api/daerahindonesia/kota?id_provinsi='.$id);
		$json = json_decode($kabs);
		$val = "<option value=''>-- Pilih kabupaten --</option>";
		if(!empty($id)) {
			foreach ($json->kota_kabupaten as $kab) {
				# code...
				$val .= "<option value='".$kab->id."'>".$kab->nama."</option>";
			}
		} else {
			$val .= "<option value=''>Data Kabupaten Tidak Ditemukan</option>";
		}
		echo $val;
		// var_dump(count($json->kabupatens));
	}
	...
	public function list_kecamatan() {
		$id = $this->input->get('id');
		$kecamatans = $this->curl->simple_get('http://dev.farizdotid.com/api/daerahindonesia/kecamatan?id_kota='.$id);
		$json = json_decode($kecamatans);
		$val = "<option value=''>-- Pilih Kecamatan --</option>";
		if(!empty($id)) {
			foreach ($json->kecamatan as $kec) {
				# code...
				$val .= "<option value='".$kec->id."'>".$kec->nama."</option>";
			}
		} else {
			$val = "<option value=''>Data Kecamatan Tidak Ditemukan</option>";
		}
		echo $val;
		// var_dump($json);
	}
	...
	public function act_daftar() {
		...
		$tgl_lahir_exp = explode("-", $tgl_lahir);
		$tgl_lahir_sql = $tgl_lahir_exp[2]."-".$tgl_lahir_exp[1]."-".$tgl_lahir_exp[0];

		$prov = strtoupper($this->input->post('provinsi'));
		$kab = strtoupper($this->input->post('kabupaten'));
		$kec = strtoupper($this->input->post('kecamatan'));
		...
		$post = [
			...
			'nohp' => $nohp,
			'alamat' => $prov.",".$kab.",".$kec,
			'tgl_lahir' => $tgl_lahir_sql
		];

		$send = $this->db->insert($tbl, $post);
		if($send) {
			$msg = ['msg' => 200];
	...
	public function daftar_sukses() {
		...
		$this->load->view('aaa_daftar_sukses', $data);
	}

	public function cek_confirm_ajax($nim) {
		$data = $this->db->query('SELECT COUNT(m.id) as ada, a.* FROM m_admin as m, m_siswa as a  WHERE m.level = "siswa" AND m.kon_id = a.id AND m.username = "'.$nim.'"')->result();
	...
	public function monitor_hasil($twk,$tiu,$tkp) {
		...
		$a['waktu_tes'] = $this->db->query("SELECT SUM(tr_guru_tes.waktu) as waktuUjuan 
			FROM tr_guru_tes 
			LEFT JOIN m_mapel ON tr_guru_tes.id_mapel = m_mapel.id")->row();

		$a['hasil'] = $this->db->query("
			SELECT m_siswa.*, tr_ikut_ujian.*
			FROM tr_ikut_ujian 
			INNER JOIN m_siswa ON tr_ikut_ujian.id_user = m_siswa.id
	...
	function generateQR($data) {
	  ...
	  $params['level'] = 'H';
		$params['size'] = 10;
		$params['savename'] = FCPATH.'/qrcode/'.$data.'.png';
	  $this->ciqrcode->generate($params);
	}
	public function kartupeserta_cetak($no_peserta) {
		$nop = encrypt_decrypt('decrypt',$no_peserta);
		$this->generateQR($nop);
		$data = [
			'qr' => base_url('qrcode/'.$nop.'.png'),
			'no_peserta' => $nop,
			'data_peserta' => $this->db->get_where('m_siswa', ['nim' => $nop])->row()
		];
