...
function login(e) {
	...
	$.ajax({
		...
		success: function(r) {
			if (r.log.status == 0) {
				// $("#konfirmasi").html("<div class='alert alert-danger d-block'>"+r.log.keterangan+"</div>");
				var n = notyfy({text: r.log.keterangan, type: 'error'});
				setTimeout(() => {
					n.close();
				}, 3000);
				$("input[type='submit']").html("Login").fadeIn('slow');
			} else {
				// $("#konfirmasi").html("<div class='alert alert-success d-block'><i class='glyphicon glyphicon-check m-r-5'></i> "+r.log.keterangan+"</div>");
				var n = notyfy({text: r.log.keterangan, type: 'success'});
				setTimeout(() => {
					n.close();
...
function m_siswa_u(id) {
	if (confirm('Anda yakin..? Akan membuat PIN ..!')) {
		$.ajax({
			type: "GET",
			url: base_url+"adm/m_siswa/user/"+id,
			success: function(response) {
